...
class DmozSpider(BaseSpider):
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(url=start_url[0], headers=self.header_vals, callback=self.parse)

    def parse(self, response):
        # print('Login form----------------------' , response.body)
        form_request = [FormRequest.from_response(response, formname='login_form',
    ...
    def after_login(self, response):
        print('After lofin response ----------------------' , response.body)
